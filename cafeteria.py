    ...
    if request.method == "POST":  
        try:  
            ...
            with sqlite3.connect("Cafeteria.db") as con:  
                cur = con.cursor()  
                cur.execute("INSERT into Usuarios (Tipo, Nombre, Cedula, Correo, Contraseña ) values (?,?,?,?,?)",(Tipo,Nombre,Cedula,Correo,Contraseña))  
                con.commit()  
                msg = "Cajero adicionado con exito"  
        except:  
            con.rollback()  
            msg = "No se pudeo adicionar el cajero a la lista"  
        finally:  
...
def savedetails1():  
    ...
    if request.method == "POST":  
        try:  
            ...
            with sqlite3.connect("Cafeteria.db") as con:  
                cur = con.cursor()  
                cur.execute("INSERT into Productos (Referencia, Nombre, Cantidad_Inventario, Valor_Unitario ) values (?,?,?,?)",(Referencia,Nombre,Cantidad,Precio))  
                con.commit()  
                msg = "Producto adicionado con exito"  
        except:  
            con.rollback()  
            msg = "No se pudeo adicionar el producto a la lista"  
        finally:  
